---
name: brand-promote
description: מחלקת קידום ופרסום — SEO, Google/YouTube Ads, Meta/TikTok Ads, יצירת תוכן ומשפיענים, וטאבולה/רכישת כתבות עבור מותג מנעולים חכמים וכספות.
---
...
## Usage
`/brand-promote [seo|search|social|content|native|status]`
...
- `content` → מפעיל `content-creator-agent`
...
## תהליך: `seo` / `search` / `social` / `content` / `native`

1. ודא ש-`marketing-manager-agent` הגדיר עדיפות/תקציב לערוץ המבוקש (אם לא — הפעל אותו קודם).
2. הפעל את agent הערוץ המתאים (`seo-agent` / `paid-search-agent` / `social-ads-agent` / `content-creator-agent` / `native-pr-agent`).
3. **חובה:** אם המשימה נוגעת לכל תוכן/קמפיין בקטגוריית כספות נשק — עצור ובקש אישור מתועד ועדכני מ-`security-compliance-agent` (מדיניות הפלטפורמה + רגולציה) לפני יצירת קריאייטיב או שקה. אין לעקוף שלב זה.
