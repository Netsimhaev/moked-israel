...
import { SafeComparisonTable } from "@/components/SafeComparisonTable";
...
export function generateStaticParams() {
  return gunSafeProducts.map((p) => ({ slug: p.slug }));
}

export default async function GunSafePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const product = getGunSafeBySlug(slug);
  if (!product) notFound();
        ...
        <section className="py-10 sm:py-14">
          <div className="mx-auto max-w-[1180px] px-6 sm:px-8">
            <p className="mb-5 text-[0.8rem] text-gray">
              ...
              &gt;{" "}
              <Link href="/safes/guns" className="text-navy-deep">
                כספות נשק
              </Link>{" "}
            ...
            <div className="grid gap-10 sm:grid-cols-[1.2fr_1fr]">
              <div>
                <SafeImageGallery images={product.images} productName={product.name} />
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-[0.82rem]">
                    <span className="rounded-full bg-cream px-3 py-1.5 text-charcoal">
                      משלוח בלבד: <b className="font-num">₪{product.shippingPrice}</b>
                    </span>
                    <span className="rounded-full bg-cream px-3 py-1.5 text-charcoal">
                      משלוח + התקנה מקצועית: <b className="font-num">₪{product.installationPrice}</b>
                    </span>
                  </div>
                </div>
            ...
            <div className="mt-12">
              ...
              <div className="mt-6">
                <SafeComparisonTable currentSlug={product.slug} />
              </div>
            </div>
            ...
            {/* ============ הוכחה חברתית ============ */}
            {/* TODO: אין עדיין עדות לקוח אמיתית לכספות הנשק — להוסיף כאן
                ציטוט אמיתי אחרי המכירות הראשונות. אין להמציא שם/ציטוט/דירוג,
                ראה product-page-agent/model-f.md ו-model-e.md */}
